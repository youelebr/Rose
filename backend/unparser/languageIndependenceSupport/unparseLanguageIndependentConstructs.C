...
// Return a string wich contain as space as column before the node
// 
...
UnparseLanguageIndependentConstructs::curprint (const std::string & str, SgLocatedNode *node) const
{
    ...
    if (is_fortran90 && 
      ...
      int used_cols = unp->cur.current_col();     // 'current_col' is zero-based
      int free_cols = usable_cols - used_cols;
      if( ! (used_cols == 0 && str[0] != ' ' ) &&  free_cols < str_len) {
        // emit fixed-format line continuation
        unp->cur.insert_newline(1);
        unp->u_sage->curprint("     &");
      }
...
UnparseLanguageIndependentConstructs::unparseExpression(SgExpression* expr, SgUnparse_Info & info)
{
  ...
  //DBG_MAQAO
  if(isSgExponentiationOp(expr)) {
    expr->get_file_info()->unsetCompilerGenerated ();
    expr->get_file_info()->unsetCompilerGenerated ();
  }

  if (expr->get_file_info()->isCompilerGenerated() != expr->get_startOfConstruct()->isCompilerGenerated() && false)
  {
    ...
    printf("\n/* Top of unparseExpression %s \n at: %s \n compiler-generated (file_info) = %s \n compiler-generated (startOfConstruct) = %s */ \n", 
            expr->class_name(), 
            *buffer, 
...
UnparseLanguageIndependentConstructs::unparseDefineDirectiveStatement (SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  ROSE_ASSERT(directive != NULL);
  curprint(directive->get_directiveString(),stmt);
  // unp->u_sage->curprint_newline(); 
  unp->cur.insert_newline(1);
}
