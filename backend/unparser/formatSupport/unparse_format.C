...
// std::cout << "num : " << num << std::endl;
  ...
  if(indent < 0 && prevnode) {
    // prevnode->get_file_info ()->get_col();
  }
...
void UnparseFormat::format(SgLocatedNode* node, SgUnparse_Info& info, FormatOpt opt) {
          ...
          default: {
            if (!info.inConditional()) {
              ...
              if  (v == V_SgFunctionDefinition || v == V_SgClassDefinition)
                ...
                insert_newline(1,currentIndent);
                // insert_newline(1,node->get_file_info()->get_col());
              else
      ...
      case FORMAT_BEFORE_BASIC_BLOCK1:
        if ( v1 != V_SgCatchOptionStmt && v1 != V_SgDoWhileStmt  && 
             ...
             v1 != V_SgSwitchStatement && v1 != V_SgWhileStmt ) { 
              // insert_newline(1); 
              // std::cout << "line : " << node->get_file_info()->get_line() << " col : " << node->get_file_info()->get_col()<< " char on line  : " << chars_on_line << std::endl;
              // insert_newline(1,node->get_file_info()->get_col());
        }else {
          insert_newline(1,currentIndent);
          // insert_newline(1,node->get_file_info()->get_col());
...
} //fin function UnparseFormat::format
