...
{
  // This is a supporexprting function for the unparseStatementNumbers, but can be called directly for statments
  ...
  if (numeric_label_exp != NULL)
  {
    ...
    if (numeric_label >= 0)
    {
     ...
     std::string numeric_label_string = StringUtility::numberToString(numeric_label);
     ...
     if (fixedFormat == true)
     {
      ...
      curprint( numeric_label_string , numeric_label_exp);
    }
    else
    {
      if (fixedFormat == true)
...
FortranCodeGeneration_locatedNode::unparseWhileStmt(SgStatement* stmt, SgUnparse_Info& info) 
{
  ...
  curprint(spaceBeforeStmt(stmt, 3));
  // curprint("DO WHILE ");
  curprint("DO ",stmt);
  #if 0
...
FortranCodeGeneration_locatedNode::unparseCaseStmt(SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  curprint(spaceBeforeStmt(stmt, 6));
  curprint("CASE (",stmt);
...
FortranCodeGeneration_locatedNode::unparseDefaultStmt(SgStatement* stmt, SgUnparse_Info& info)
{
  // Sage node corresponds to Fortran 'case default'
       ...
       curprint(spaceBeforeStmt(stmt, 12));
       curprint("CASE DEFAULT",stmt);
...
FortranCodeGeneration_locatedNode::unparseExprStmt(SgStatement* stmt, SgUnparse_Info& info) 
{
  ...
  if ( ! isSgFunctionCallExp(expr_stmt->get_expression()) && !ninfo.inVarDecl()) 
  { 
    SgNode * closeScope = expr_stmt->get_parent();
    while (!isSgBasicBlock(closeScope)) closeScope = closeScope->get_parent();

    int space = closeScope->get_parent()->get_file_info()->get_col();
    curprint(spaceBeforeStmt(space+2));
  }
...
FortranCodeGeneration_locatedNode::unparseVarDecl(SgStatement* stmt, SgInitializedName* initializedName, SgUnparse_Info& info)
{
  ...
  // printf ("In unparseVarDecl(SgStatement,SgInitializedName,SgUnparse_Info): info.SkipBaseType() = %s \n",info.SkipBaseType() ? "true" : "false");
    ...
    if (numVar > 1) {
      ...
      unp->u_fortran_type->unparseType(baseType, info, false, stmt);  // do not print type attributes such as dimension, length on the left of ::
    }
    else {
      unp->u_fortran_type->unparseType(type, info, true, stmt);  // print type attribute on the left of ::
    }
...
void FortranCodeGeneration_locatedNode::curprint(const std::string & str, SgLocatedNode * node) const {
  ...
  unp->u_sage->curprint(str);
    ...
    //std::cout << "Current Line : "<< unp->cur.current_line() <<" col : " << curr_line_len << "str : "<< str <<  std::endl;
    if (is_fortran90 && 
      curr_line_len !=0 && 
