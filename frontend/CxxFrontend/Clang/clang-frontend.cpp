...
//          inc_dirs_list.insert(inc_dirs_list.begin(), c_config_include_dirs.begin(), c_config_include_dirs.end());
    ...
    std::cout << "ARGS | i "<< i << " cnt " << cnt << std::endl;
    for( int i = 0 ; i <= cnt ; i++) {
...
DBG_MAQAO
  // 2 - Create a compiler instance
    ...
    // compiler_instance->getLangOpts().CPlusPlus0x = 1;

DBG_MAQAO

    // #if CLANG38
    // clang::TextDiagnosticPrinter * diag_printer = new clang::TextDiagnosticPrinter(llvm::errs(), &clang::DiagnosticOptions(),false);
    // #else
    clang::TextDiagnosticPrinter * diag_printer = new clang::TextDiagnosticPrinter(llvm::errs(), clang::DiagnosticOptions());
    // #endif

    // #if CLANG38
    // compiler_instance->createDiagnostics(new DiagnosticOptions);
    // #else
...
DBG_MAQAO
    ...
    switch (language) {
      ...
      case ClangToSageTranslator::CPLUSPLUS:
        ...
        // lang_opts.CPlusPlus0x = 1;
        compiler_instance->getLangOpts().CPlusPlus = 1; // solved c++ problem 'undefined reference to bool'
        // compiler_instance->getLangOpts().CPlusPlus0x = 1;
        // compiler_instance->getInvocation().setLangDefaults(compiler_instance->getLangOpts(), clang::IK_CXX );
        // compiler_instance->getHeaderSearchOpts().UseLibcxx = 1;
        // compiler_instance->getHeaderSearchOpts().UseStandardCXXIncludes = 1;
        // compiler_instance->getHeaderSearchOpts().UseStandardSystemIncludes = true;
    ...
    if (!compiler_instance->hasPreprocessor()) {DBG_MAQAO compiler_instance->createPreprocessor(); }
    else {DBG_MAQAO}
    // if (!compiler_instance->hasPreprocessor()) compiler_instance->createPreprocessor(clang::TU_Complete);

    if (!compiler_instance->hasASTContext()) {DBG_MAQAO compiler_instance->createASTContext(); }
    else {DBG_MAQAO}
...
compiler_instance->getInvocation().toArgs(args_vector);
std::cerr << "vector to args :" << std::endl;
for(int i_av=0; i_av < args_vector.size(); i_av++)
std::cerr << args_vector[i_av] << std::endl;

std::cerr << "Unknown : " << compiler_instance->getLangOpts().CPlusPlus << " !" << std::endl;


// sleep(5);
    //clang::ParseAST(compiler_instance->getPreprocessor(), &translator, compiler_instance->getASTContext());
    clang::ParseAST(compiler_instance->getSema());
std::cout << " -----------------------------------------------------------------------------------------------------------------------------" << std::endl;
// exit(1);
DBG_MAQAO
...
// #ifdef CLANG38
void SagePreprocessorRecord::PragmaDiagnostic(clang::SourceLocation Loc, llvm::StringRef Namespace, clang::diag::Severity mapping, llvm::StringRef Str) {
