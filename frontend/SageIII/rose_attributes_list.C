...
#define DBG_MAQAO cerr<<"In "<<__FILE__<<"FUNCTION "<<__FUNCTION__<<" , line: "<<__LINE__<<"\n"; //DBG_MAQAO
...
int ROSEAttributesList::isFortran90Comment( const string & line ) {
  //DBG_MAQAO
  ...
  if (firstNonBlankCharacter == '!' || firstNonBlankCharacter == '\n') {
    ...
    //Si le commentaire est en fin de ligne (et non dans une chaine de charactères)
    //Premier charactère différent de "!" et d'un blanc ou d'une tabulation.
    --i;
    while (i < lineLength) {
      i++;
      firstNonBlankCharacter = line[i];
      ...
      if(firstNonBlankCharacter == '"') {
        //On prend le charactère suivant
        firstNonBlankCharacter = line[++i]; 
        //tant qu'il n'y a pas d'autre '"' on continue de parcourir la chaine de charactère
        while (i < lineLength && firstNonBlankCharacter != '"') { firstNonBlankCharacter = line[++i]; }
      }
      if (firstNonBlankCharacter == '!') {
        // printf ("This is a F90 style comment: line = %s length = %zu \n",line.c_str(),line.length());
        isComment = i;
...
ROSEAttributesList::collectPreprocessorDirectivesAndCommentsForAST( const string & filename, ROSEAttributesList::languageTypeEnum languageType )
{
  ...
  //std::cerr << "filename-cerr : " << filename << std::endl;
  //std::cout << "filename-cout : " << filename << std::endl;
  ...
  if (targetFile.is_open()) {
    ...
    while ( targetFile.eof() == false ) {
      ...
      bool cppDirective_b = isCppDirective(line,cppDeclarationKind,restOfTheLine);

      #if DEBUG_CPP_DIRECTIVE_COLLECTION
       printf ("cppDirective_b = %s cppDeclarationKind = %s \n",cppDirective_b ? "true" : "false",PreprocessingInfo::directiveTypeName(cppDeclarationKind).c_str());
      #endif

      if (cppDirective_b == true) {
        #if 0
      ...
      if (cppDirective_b == false) {
        ...
        switch (languageType) {
          ...
          case e_C_language: isComment = -1; break;
          case e_Cxx_language: isComment = -1; break;
      ...
      if (cppDeclarationKind != PreprocessingInfo::CpreprocessorUnknownDeclaration) {
        //if(cppDirective_b == true) {
        //DBG_MAQAO
        //cout << "isComment = " << isComment << endl;
        //}
        PreprocessingInfo* cppDirective;
        if(isComment == 0 || cppDirective_b == true)
          cppDirective= new PreprocessingInfo(cppDeclarationKind,line,filename,
                                              lineCounter,0,numberOfLines,PreprocessingInfo::before);
        else if(isComment != -1) {
          //Le commentaire est à la fin de la ligne
          string subLine = line.substr (isComment);
          //std::cout<< "\n\n\n\n\n substring : " << subLine << std::endl << "string : "<< line << std::endl;
          cppDirective= new PreprocessingInfo(cppDeclarationKind,subLine,filename,
                                              lineCounter,isComment,numberOfLines,PreprocessingInfo::before);
        }

        //DBG_MAQAO
        #if 0
        if (cppDirective != NULL) {
          //printf("/!\\ Error Syntax : \" %s \" is unknown (not a directive or a comment) the file will not be analyzed", line);
