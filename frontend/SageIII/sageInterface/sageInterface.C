  ...
  for (SgStatementPtrList::iterator i = statementList.begin(); i != statementList.end(); i++)
  {
    ...
    if (declaration != NULL)
    {
      ...
      switch(declaration->variantT())
      {
        ...
        case V_SgUseStatement: {
          SgUseStatement * usestmt = isSgUseStatement(declaration);

          SgModuleStatement * moduleStatement = isSgModuleStatement(usestmt->get_module ());
          SgClassDefinition* classDefinition = moduleStatement->get_definition();
          if (classDefinition) {
            SgSymbol* symbol = classDefinition->first_any_symbol();
            SgRenamePairPtrList only_options = usestmt->get_rename_list();
            while (symbol != NULL) {
              SgAliasSymbol* aliasSymbol = new SgAliasSymbol(symbol,/* isRenamed */ false);
              scope->insert_symbol(aliasSymbol->get_name (), aliasSymbol);
        ...
        default:
        {
          ...
          std::cout << "class name : " << declaration->class_name() << std::endl;
          ROSE_ASSERT(false);
        }
      }
...
void SageInterface::replaceExpression(SgExpression* oldExp, SgExpression* newExp, bool keepOldExp/*=false*/)
{
  ...
  {
    ROSE_ASSERT (isSgFortranDo(parent)->get_bound() == oldExp);
    isSgFortranDo(parent)->set_bound(newExp);
  }
  ...
  // Fortran used
  else if (isSgSubscriptExpression (parent)) 
  {
    if (oldExp==isSgSubscriptExpression(parent)->get_lowerBound()) {
      isSgSubscriptExpression(parent)->set_lowerBound(newExp);
    } else if (oldExp==isSgSubscriptExpression(parent)->get_upperBound()) {
      isSgSubscriptExpression(parent)->set_upperBound(newExp);
    } else if (oldExp==isSgSubscriptExpression(parent)->get_stride()) {
      isSgSubscriptExpression(parent)->set_stride(newExp);
    } else  {
      ROSE_ASSERT(false);
    }
  }
...
bool SageInterface::loopTiling(SgForStatement* loopNest, size_t targetLevel, size_t tileSize)
{
  ...
  string ivar2_name = "lt_var_"+ivar->get_name().getString();
...
SgAssignInitializer* SageInterface::splitExpression(SgExpression* from, string newName/* ="" */)
{
  ...
  bool SageInterface::isConstantFalse(SgExpression* e) {
    switch (e->variantT()) {
      ...
      case V_SgIntVal: return isSgIntVal(e)->get_value() == 0;
      case V_SgCastExp: return isConstantFalse(isSgCastExp(e)->get_operand());
