...
setSourcePosition  ( SgInitializedName* initializedName, Token_t* token )
   {
    //DBG_MAQAO
     if ( SgProject::get_verbose() > 1 )
          printf ("In setSourcePosition initializedName = %p = %s token = %p line = %d col = %d \n",initializedName,initializedName->get_name().str(),token,token != NULL ? token->line : -1,token != NULL ? token->col : -1);
    ...
    //DBG_MAQAO
     if ( SgProject::get_verbose() > 0 )
       ...
       // printf ("In setSourcePosition locatedNode = %p = %s token = %p line = %d \n",locatedNode,locatedNode->class_name().c_str(),token,token != NULL ? token->line : -1);
          printf ("In setSourcePosition locatedNode = %p = %s token = %p (file = %s) line = %d col = %d\n",locatedNode,locatedNode->class_name().c_str(),token,getCurrentFilename().c_str(),token != NULL ? token->line : -1,token != NULL ? token->col : -1);
        }
...
SgClassSymbol* 
buildIntrinsicModule_ISO_FORTRAN_ENV()
   {
    //DBG
     SgClassSymbol* moduleSymbol = NULL;

     const string name = "ISO_FORTRAN_ENV";

     if ( SgProject::get_verbose() > DEBUG_COMMENT_LEVEL )
          printf ("Building the module file for ISO_FORTRAN_ENV \n");
  ...
  // are no others that I know of.
     ...
     if (matchingName(name,"ISO_FORTRAN_ENV") == true)
       ...
       // This is the ISO_FORTRAN_ENV intrisic module and so there is a list of names and types that need to be inserted into the local scope.
          ...
          //#endif
          moduleSymbol = buildIntrinsicModule_ISO_FORTRAN_ENV();
        }
