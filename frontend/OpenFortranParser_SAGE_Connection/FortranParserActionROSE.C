...
// void c_action_private_components_stmt(Token_t * label)
    void c_action_private_components_stmt(Token_t *label, Token_t *privateKeyword,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_binding_private_stmt(Token_t * label)
    void c_action_binding_private_stmt(Token_t *label, Token_t *privateKeyword,
            Token_t *eos)
    {   
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_enumerator_def_stmt(Token_t * label)
    void c_action_enumerator_def_stmt(Token_t *label, Token_t *enumeratorKeyword,
            Token_t *eos)
    {
   //DBG_MAQAO
    }
...
void c_action_declaration_type_spec(Token_t * udtKeyword, int type)
{
  ...
  switch (type)
  {
    ...
    case DeclarationTypeSpec_unlimited:
    {
      ...
      printf("line %d | col %d | text %s\n",udtKeyword->line, udtKeyword->col,udtKeyword->text );
      printf("In R502 c_action_declaration_type_spec() udtKeyword = %p = %s type = %d \n",
            udtKeyword, udtKeyword != NULL ? udtKeyword->text : "NULL",
            type);
      ROSE_ASSERT(false);
        ...
        default:
        {
          //if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) 
            printf("In R503 c_action_attr_spec(): attrKeyword = %p = %s attr = %d \n",
                attrKeyword, attrKeyword != NULL ? attrKeyword->text : "NULL",
                attr);
            printf("line : %d col :  %d \n",attrKeyword->line, attrKeyword->col);
...
// void c_action_equivalence_stmt(Token_t * label)
    void c_action_equivalence_stmt(Token_t *label, Token_t *equivalenceKeyword,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// public abstract void target_decl_list(int count);
   ...
   //DBG_MAQAO
        ...
        if (astExpressionStack.size() != (size_t) count) {
            printf("astExpressionStack size = %d | count = %d\n",astExpressionStack.size(), (size_t) count);
        }
...
void c_action_common_block_object(Token_t *id, ofp_bool hasShapeSpecList)
{
  ...
  else
  {
    ...
    // OR it maybe declared at some point in the future (see test2010_51.90).
      ...
      // Now verify that it is present.
      variableSymbol = trace_back_through_parent_scopes_lookup_variable_symbol(
...
void c_action_data_ref(int numPartRef)
{
  ...
  for (int i = 0; i < numPartRef; i++)
  {
    ...
    // DXN (10/08/2012): Look for the variable symbol for each qualified name in the appropriate scope
    SgScopeStatement* structureScope = getTopOfScopeStack();
    for (size_t i = 0; i < qualifiedNameList.size(); i++)
    {
      ...
      trace_back_through_parent_scopes_lookup_variable_symbol_but_do_not_build_variable(qualifiedNameList[i].name, structureScope, varSym, funcSym, classSym);
      if (varSym)
        findStructureScope(varSym->get_type(), structureScope);
      else
        structureScope = getTopOfScopeStack();
      
      if (qualifiedNameList[i].hasSelectionSubscriptList)
      {
        if (varSym != NULL)
        {
          ...
          SgInitializedName* variableName = varSym->get_declaration();
          ...
          if (!arrayType && !stringType && !functionType)
          {
            ...
            ROSE_ASSERT(matchingName(variableName->get_name().str(),qualifiedNameList[i].name));
            ...
            scope->remove_symbol(varSym);
          }
        } // end varSym != NULL
      } // end qualifiedNameList[i].hasSelectionSubscriptList
    } // end for
        ...
        if (isAnIntrinsicFunction == true)
        {
          ...
          if (astExpressionStack.empty() == true)
          {
            ...
            else
            {
              ...
              if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
                cout << "Case ExpressionStack is empty and with implicit none: after generateFunctionCall, variableSymbolList.size() = "
                     << variableSymbolList.size() << endl;
          ...
          else
          {
            ...
            if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
              cout << "Case ExpressionStack is not empty: after generateFunctionCall, variableSymbolList.size() = "
                   << variableSymbolList.size() << endl;
    ...
    else // variableSymbol != NULL
    {    // The variable already existed
         //Often append in Fortran77
      ...
      #endif
      // if (initializedName->get_scope()->get_symbol_table()->find(initializedName) == NULL) {
        // printf("Found a previously defined variableSymbol = %s, line = %d\n", variableName.str(), variableSymbol->get_declaration()->get_file_info()->get_line());
      // }
  ...
  outputState("At BOTTOM of R612 c_action_data_ref()");
   ...
   //DBG_MAQAO
...
#if 0
        // DXN: this chunk of code is essentially moved to c_action_data_ref because only in data_ref that we have
        // enough information to determine the scope of each of the part refs and look for each of them in the
        // appropriate scope.
        SgName name = id->text;
        SgVariableSymbol* variableSymbol = NULL;
        SgFunctionSymbol* functionSymbol = NULL;
        SgClassSymbol* classSymbol = NULL;
        trace_back_through_parent_scopes_lookup_variable_symbol_but_do_not_build_variable(
                name, getTopOfScopeStack(), variableSymbol, functionSymbol, classSymbol);
...
#endif  // DXN: chunk of code to be removed
...
// void c_action_where_construct_stmt(Token_t * id)
    void c_action_where_construct_stmt(Token_t *id, Token_t *whereKeyword,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_cycle_stmt(Token_t * label, Token_t * id)
    void c_action_cycle_stmt(Token_t *label, Token_t *cycleKeyword, Token_t *id,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_exit_stmt(Token_t * label, Token_t * id)
    void c_action_exit_stmt(Token_t *label, Token_t *exitKeyword, Token_t *id,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_stop_stmt(Token_t * label, ofp_bool hasStopCode)
   ...
   //DBG_MAQAO
        printf(
                "In c_action_allstop_stmt() - this function needs to be implemented.\n");
    }
   ...
   //DBG_MAQAO
        printf(
                "In c_action_sync_all_stmt() - this function needs to be implemented.\n");
    }
...
// void c_action_format_stmt(Token_t * label)
   ...
   //DBG_MAQAO
        ...
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
            printf("In c_action_program_stmt(): label = %s programKeyword = %s id = %s \n",
                    label ? label->text : "NULL",programKeyword ? programKeyword->text : "NULL", id ? id->text : "NULL");
                        ...
                        ofp_bool hasRenameList, ofp_bool hasOnly)
{
   ...
   if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    ...
    #if 0 
      // before to change to accept "USE" stmt without a file with the same name
    ...
    if (moduleSymbol == NULL) {
      ...
      //std::cout << "Use : type : " << useStatement->unparseToString() << std::endl;
      if (moduleStatement == NULL) {
         //cout << "Error : cannot find the module (module should have been seen in a previously generated file), the declaration will not have a definition : "
         //   << modName << endl;
         //ROSE_ASSERT(false);
         //----------------NEW CODE
         //Correct this ! it's NULL and it have to be type of "struct" 
         SgClassType ct(useStatement);

         //std::cout << "useStatement TYPE : " << (useStatement->get_type ())->unparseToString() << std::endl;

         SgModuleStatement* moduleStatement2 = new SgModuleStatement(useStatement->get_name(), //SgName
            SgClassDeclaration::e_struct, // SgClassDeclaration::class_types 1 == struct
            &ct,  // SgClassType
            NULL) // SgClassDefinition
         ;

        moduleStatement2->set_scope(SageInterface::getGlobalScope(currentScope));
        moduleStatement2->set_file_info(currentScope->get_file_info());
        moduleStatement2->set_parent(SageInterface::getGlobalScope(currentScope));

        moduleStatement = isSgModuleStatement(moduleStatement2);
        //----------------END NEW CODE
      }
   ...
   // std::cout << "ALL ABOUT MODULE : " << std::endl;
   // std::cout << "Module name : " << moduleStatement->get_name() << std::endl;
   // std::cout << "class_type : " << moduleStatement->get_class_type ()  << std::endl;
   // //std::cout << "\nSgClassType& : " << &moduleStatement->get_type () << std::endl;
   // std::cout << "SgClassType : " << (moduleStatement->get_type ())->unparseToString() << std::endl;
   // std::cout << "SgClassDef : NULL" << std::endl;
...
void c_action_import_stmt(Token_t *label, Token_t *importKeyword, Token_t *eos,ofp_bool hasGenericNameList)
{
  ...
  while (astNameStack.empty() == false)
  {
    ...
    SgVariableSymbol* variableSymbol =
    trace_back_through_parent_scopes_lookup_variable_symbol(importName, astScopeStack.front());
    if (variableSymbol == NULL) {
      printf("In c_action_import_stmt(): importKeyword = %p = %s hasGenericNameList = %s \n",
          importKeyword,
          importKeyword != NULL ? importKeyword->text : "NULL",
          hasGenericNameList ? "true" : "false");
      std::cout << "line : " << importKeyword->line << "col : " << importKeyword->col << std::endl;
    }
...
// void c_action_external_stmt(Token_t * label)
    void c_action_external_stmt(Token_t *label, Token_t *externalKeyword,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// void c_action_intrinsic_stmt(Token_t * label)
    void c_action_intrinsic_stmt(Token_t *label, Token_t *intrinsicToken,
            Token_t *eos)
    {
   //DBG_MAQAO
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
// FMZ (2/9/2009): possible cases: 
     ...
     */
    void c_action_rice_co_dereference_op(Token_t *leftBracket,
            Token_t *rightBracket)
    {
...
// It appears that it is strict.
    ...
    void c_action_specific_binding(Token_t *carg_0, Token_t *carg_1,
            Token_t *carg_2, Token_t *carg_3, ofp_bool carg_4)
    {
    }
...
void c_action_next_token(Token_t *token)
{
   //DBG_MAQAO 
        ...
        string text = token->text;
        std::cout << "Next Token : " << text << std::endl;
